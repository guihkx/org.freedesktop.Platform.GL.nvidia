...
for installation_path in "${installation_paths[@]}"; do
  ...
  test_file="$(printf './tests/R%s-%s-%s-expected-files.txt' "${DRIVER_VERSION_MAJOR}" "${DRIVER_ARCH}" "${ext_suffix}")"
  echo -n "info: generating test file for driver ${DRIVER_VERSION} (${arch})... "
  (cd "${installation_path}" && find . ! -path . ! -path ./.ref ! -path ./manifest.json \( -type l -printf '%P (%Y) -> %l | %#m (%M)\n' -o -printf '%P | %#m (%M)\n' \) | LC_ALL=C sort -fV | perl -pe "s/\Q${DRIVER_VERSION}\E|\Q${DRIVER_VERSION_DASH}\E/${DRIVER_VERSION_MAJOR}/g") > "${test_file}"
  echo "done"
done
